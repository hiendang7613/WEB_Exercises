...
window.onload = function () {
  var chess_pieces = document.getElementsByClassName("chess-piece");
  Array.from(chess_pieces).forEach((element, index) => {
    if (index < 8) {
...
function IsKick(obj, Col, Row) {
  var elements = document.getElementsByClassName("chess-piece");
...
function IsBlocked(obj, dCol, dRow) {
  ...
  return Array.from(chess_pieces).some((element) => {
    ...
    //console.log(dCol2 * dCol <= 0 && dRow * dRow2 <= 0);
    //if () return false;
